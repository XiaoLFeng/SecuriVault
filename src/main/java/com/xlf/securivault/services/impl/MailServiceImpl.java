...
public class MailServiceImpl implements MailService {
     ...
     *
     * @param request      请求
     * @param generateCode 生成的验证码
     * @return 链接
     */
    private static @NotNull StringBuffer getLink(@NotNull HttpServletRequest request, String generateCode) {
    ...
    @Async
    @Override
    public void sendMail(String email, String template, @NotNull Map<String, Object> parameter) {
    ...
    public void sendResetPasswordMail(String mailTo, HttpServletRequest request) {
        ...
        if (getUser != null) {
            ...
            StringBuffer createLink = getLink(request, generateCode);
            parameter.put("link", createLink);
            parameter.put("email", mailTo);
            parameter.put("username", getUser.getUsername());
            // 发送邮件
            this.sendMail(mailTo, "user-reset-password", parameter);
        } else {
     ...
     * 发送用户授权邮件
     * <hr/>
     * 用于发送用户授权邮件；根据邮件模板和邮件接收者发送用户授权邮件；
     *
     * @param getUser 用户
     */
    @Override
    public void sendUserAuthorizationMail(@NotNull UserDO getUser, HttpServletRequest request) {
        // 检查是否可以再次发送邮件
        this.checkSendTime(getUser.getEmail());
        // 生成随机验证码
        String generateCode = Util.generateRandString(6);
        // 保存验证码
        EmailVerifyCodeDO newEmailVerifyCode = new EmailVerifyCodeDO();
        newEmailVerifyCode
        ...
        // 生成 Map 信息
        HashMap<String, Object> parameter = new HashMap<>();
        parameter.put("code", generateCode);
        parameter.put("username", getUser.getUsername());
        // 发送邮件
        this.sendMail(getUser.getEmail(), "user-send-authorization", parameter);
    }
     ...
     * @param mail 邮箱
     */
    private void checkSendTime(String mail) {
        EmailVerifyCodeDO getEmailVerifyCode = emailVerifyCodeDAO.getLastVerifyCodeByEmail(mail);
