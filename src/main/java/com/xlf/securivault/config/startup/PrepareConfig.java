...
public class PrepareConfig {
     ...
     * <hr/>
     * 用于处理服务缺失对应数据表处理的资源；
     * 用于检查是否需要生成对应 info 的内容
     *
     * @return CommandLineRunner
     */
    @Bean
    @Order(5)
    public CommandLineRunner prepareFourth() {
        return args -> {
            log.info("[STARTUP] 检查是否需要生成对应 info 的内容");
            prepare.tableInfo("title", "SecuriVault");
            prepare.tableInfo("initialize", "0");
        };
    }
    ...
    public CommandLineRunner prepareEnv() {
        return args -> {
            ...
            SystemConfigurationVariable.setInitialize(jdbcTemplate.queryForObject(
                    "SELECT value FROM xf_info WHERE key = 'initialize'",
                    Boolean.class)
            );
        };
    }
