...
class PrepareAlgorithm {
    ...
    public void roleCheck(String roleName, String displayName, String description) {
        try {
            jdbcTemplate.queryForObject("SELECT name FROM xf_role WHERE name = ?", String.class, roleName);
        } catch (DataAccessException e) {
    ...
    public void tableInfo(String key, String value) {
        try {
            jdbcTemplate.queryForObject("SELECT id FROM public.xf_info WHERE key = ?", String.class, key);
        } catch (DataAccessException e) {
            log.info("[STARTUP] 在数据表 xf_info 中插入 {}: {}", key, value);
            jdbcTemplate.update(" INSERT INTO xf_info (key, value) VALUES (?, ?)", key, value);
        }
    }
}
